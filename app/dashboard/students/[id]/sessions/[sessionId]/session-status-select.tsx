...
type Props = {
  sessionId: string;
  studentId: string;
  currentStatus: EditableSessionStatus;
};
...
export function SessionStatusSelect({ sessionId, studentId, currentStatus }: Props) {
  ...
  function handleChange(e: React.ChangeEvent<HTMLSelectElement>) {
    const value = e.target.value as EditableSessionStatus;
    if (!EDITABLE_SESSION_STATUSES.includes(value)) return;
  ...
  return (
    ...
    >
      {EDITABLE_SESSION_STATUSES.map((s) => (
